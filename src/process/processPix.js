...
export default {
  async processaPix(keyPagamento) {
    ...
    for(let eve in ingressos) {
      ...
      if(ingressos[eve].aceita_pix == 'S' && ingressos[eve].aceita_pix_aarin == 'S') {
        gateway = 'AARIN';
      } else {
        gateway = keyPagamento.gateway_cartao;
      }
    ...
    if(gateway == "MPTP" || gateway == "MPMKT") {
      dateOfExpiration = getDateOfExpiration(2);
    }
